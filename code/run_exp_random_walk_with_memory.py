...
def get_point(
        *,
        ...
        perturbation: float,
        n_states: int,
        n_holding_states: int,
        E_length: float,
):
    assert 0 <= perturbation <= 1
    target_model = RandomWalk(
        n_states=n_states,
        E_length=E_length,
    )
    sample_model = RandomWalkWithHolding(
        n_states=n_states,
        E_length=E_length,
        holding_probability=perturbation,
        n_holding_states=n_holding_states,
    )
...
experiment_name = 'exp_random_walk_with_memory'
...
def run_experiment(*, test_run):
    ...
    if test_run:
        ...
        parameters_to_vary = {
            ...
            # 'n_holding_states': [1, 2, 3],
            'perturbation': np.linspace(0, 0.25, 9),
            'N_sequences': np.linspace(2, 50, 9, dtype=int),
        }
