...
from .hypothesis_tests import AsymptoticLikelihoodRatioTest
...
def get_point(
    ...
    for kernel in (
        ...
        tests |= {
            f'MMD-Wild(k={kernel[0]}, n=1000)': hold(MMDTestWildBootstrap)(
                kernel=kernel[1],
                n_mmd_samples=1_000,
    ...
    tests |= {
        ...
        # r'LR-Asymp($k=k_\mathrm{model}$)': hold(AsymptoticLikelihoodRatioTest)(
        #     target_model=target_model,
        #     get_mle_model=lambda xs: NgramModel.fit(n_in_ngram=markov_order, n_states=n_states, sequences=xs),
        #     parametric_family_param_count=(n_states - 1) * n_states ** markov_order,
        #     **test_kwargs,
        # ),
        # r'LR-Asymp(oracle)': hold(AsymptoticLikelihoodRatioTest)(
        #     target_model=target_model,
        #     get_mle_model=lambda xs: sample_model,
        #     parametric_family_param_count=1,
        #     **test_kwargs,
        # ),
    }
    point = dict(
...
experiment_name = 'exp_binary_markov_misspecified_moresamples'
