...
def get_point(
        *,
        ...
        perturbation: float,
        n_states: int,
        E_length_target: float,
        E_length_sample_if_fully_perturbed: float,
):
    assert 0 <= perturbation <= 1
    target_model = RandomNgramModelWithFixedInitDist(
        rng=get_rng(seed, 'random-ngram', 3, nth_usage=1),
        n_in_ngram=1,
        n_states=n_states,
        E_length=E_length_target,
    ...
    sample_model = RandomNgramModelWithFixedInitDist(
        ...
        n_in_ngram=1,
        n_states=n_states,
        E_length=E_length_sample_if_fully_perturbed * perturbation + (1 - perturbation) * E_length_target,
        concentration_param=1,
        initial_dist=np.ones(10) / 10,
    )
...
experiment_name = 'exp_random_mc_varied_length_dist'
...
def run_experiment(*, test_run):
    ...
    baseline_parameters = {
        ...
        'n_states': 10,
        'E_length_target': 8,
        'E_length_sample_if_fully_perturbed': 20,
    }
    if test_run:
        baseline_parameters |= {
            'N_indep_tests': 1,
    ...
    root_seed = 110163921462081850884303923825184125464, ''
    results = dict()
    ...
    def go():
        for parameter, values in parameters_to_vary.items():
            ...
            for value in values:
                ...
                point = get_point(
                    seed=extend_seed(root_seed, f'{parameter}/{value}/for-get_point'),
