...
def get_point(
    ...
    target_model = RandomNgramModel(
        rng=get_rng(seed, 'random-ngram', 1, nth_usage=nth_call + 1),
        n_in_ngram=markov_order,
        concentration_param=1,
        n_states=n_states,
        E_length=E_length,
    )
    sample_model = RandomNgramModel(
        rng=get_rng(seed, 'random-ngram', 2, nth_usage=nth_call + 1),
        n_in_ngram=markov_order,
        concentration_param=1,
        n_states=n_states,
        E_length=E_length,
...
experiment_name = 'exp_snd_order_mc_few_long_samples'
...
def run_experiment(*, test_run):
    ...
    if test_run:
        ...
        parameters_to_vary = {
            ...
            'E_length': np.linspace(2, 20, 9),
        }

    root_seed = 286940871158086836600272500561603995549, ''
    results = dict()
    ...
    def go():
        for parameter, values in parameters_to_vary.items():
            ...
            for i, value in enumerate(values):
                ...
                point = get_point(
                    seed=extend_seed(root_seed, f'{parameter}/get_point'),
                    nth_call=i,
